...
class DataCollatorForMultiTaskSeq2Seq:
    ...
    def __call__(self, features):
        ...
        input_ids_t2s = (
            [feature["input_ids_t2s"] for feature in features]
            if "input_ids_t2s" in features[0].keys()
            else None
        )
        input_ids_s2t = (
            [feature["input_ids_s2t"] for feature in features]
            if "input_ids_s2t" in features[0].keys()
            else None
        )
        ...
        if labels_t2s is not None:
            ...
            max_input_length = max(
                max(len(l) for l in input_ids_t2s), max(len(l) for l in input_ids_s2t)
            )
            ...
            for feature in features:
                ...
                # pad inputs
                remainder_t2s = [self.input_pad_token_id] * (
                    max_input_length - len(feature["input_ids_t2s"])
                )
                feature["input_ids_t2s"] = (
                    feature["input_ids_t2s"] + remainder_t2s
                    if padding_side == "right"
                    else remainder + feature["input_ids_t2s"]
                )
                ...
                # pad attention mask
                feature["attention_mask_t2s"] = (
                    feature["attention_mask_t2s"] + remainder_t2s
                )
        ...
        if self.model is not None and hasattr(
            ...
            decoder_input_ids_t2s = self.model.prepare_decoder_input_ids_from_labels(
                labels=features["labels_t2s"]
            )
            decoder_input_ids_s2t = self.model.prepare_decoder_input_ids_from_labels(
                labels=features["labels_s2t"]
            )
...
class MultiTaskSeq2SeqTrainer(Seq2SeqTrainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # exit()
        outputs_t2s = model(
            input_ids=inputs["input_ids_t2s"],
        ...
        cross_att = torch.mean(
            torch.mean(outputs_t2s.cross_attentions[-1], dim=2), dim=1
        ...
        if labels_t2s is not None:
            ...
            masked_lm_loss_t2s = loss_fct(
                lm_logits_t2s.view(-1, self.model.config.vocab_size),
                inputs["labels_t2s"].view(-1),
            )
            loss_mask_t2s = torch.ne(inputs["labels_t2s"], -100)
            masked_lm_loss_t2s = torch.sum(
                masked_lm_loss_t2s.view(inputs["labels_t2s"].shape[0], -1), dim=-1
            ) / torch.sum(
                loss_mask_t2s, dim=-1
            )  # (bs)

            lm_logits_s2t = outputs_s2t["logits"]  # (bs, seq_len, vocab_size)
            masked_lm_loss_s2t = loss_fct(
                lm_logits_s2t.view(-1, self.model.config.vocab_size),
                inputs["labels_s2t"].view(-1),
            )
            # print(masked_lm_loss_s2t)
            # loss_mask_s2t = torch.ne(inputs['labels_s2t'], -100)
            masked_lm_loss_s2t, _ = torch.max(
                masked_lm_loss_s2t.view(inputs["labels_s2t"].shape[0], -1), dim=-1
            )  # / torch.sum(loss_mask_s2t, dim=-1)  # (bs)
            # weight_s2t = (F.softmax(masked_lm_loss_s2t) * inputs['labels_s2t'].shape[0]).detach()
            if inputs["labels_s2t"].shape[0] > 1:
                weight_s2t = (
                    (1 - F.softmax(masked_lm_loss_s2t))
                    * inputs["labels_s2t"].shape[0]
                    / (inputs["labels_s2t"].shape[0] - 1)
                ).detach()
            else:
        ...
        return (
            (loss, (loss_t2s, loss_s2t, loss_att), (outputs_t2s, outputs_s2t))
            if return_outputs
            else (loss, (loss_t2s, loss_s2t, loss_att))
        )
